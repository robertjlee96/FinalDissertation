...
With the tool of BEH created, the electroweak gauge theory could finally be constructed in a way consistent with observations of previous experiments. In 1967, Weinberg and Salam built on the work Glashow had done 6 years prior by inserting the BEH spontaneous symmetry breaking into the existing attempts at electroweak unification \cite{CMS10YearReview}. The electroweak gauge theory is invariant under the $SU(3)_L x U(1)_Y$ symmetry group, where the L signifies the doublets of left-handed chirality, and Y signifies the weak hypercharge scalar \cite{PDGReview}.

Just as a field with a potential given in equation \ref{eqn:BEHLagrangian} can be inserted into the gauged version of the Goldstone theory, a field can be inserted into the electroweak lagrangian to spontaneously break the symmetry of the $SU(3)_L x U(1)_Y$ group. This field is usually written in the form given by equation \ref{eqn:HiggsPotential}.
...
The new objective function can then be written schematically as shown in equation \ref{eqn:BasicObjective} \cite{XGBoostPaper}. Here, $L(\vec{\theta})$ is a loss function, and $\Omega(\vec{\theta})$ is the regularization term. Beginning with the loss function, a form must be selected. While the mean squared error is a common option, so-called logistic loss is commonly used in binary classification. The form of this loss function can be seen below in equation \ref{eqn:LogisticLoss} \cite{XGBoostDocumentation}. 

% \begin{equation}\label{eqn:LogisticLoss}
%     L(\vec{\theta}) = \sum_{i} [y_i ln(1 + e^{- \hat{y_i}}) + (1-y_i)ln(1 + e^{\hat{y_i}})]
% \end{equation}

% So, for each data point, the closeness of the predicted probability is compared to the actual label. Immediately, it can be seen that this logistic loss ascribes a very high loss value for predictions that vary considerably from the true label of an event. In the case where $\hat{y_i} = y_i$, the loss function is minimized (as one would hope). For 
